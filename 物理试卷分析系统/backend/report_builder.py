...
from .knowledge import SECTION_INDEX, knowledge_desc, section_hot_kps
...
def build_html(analysis: dict, meta: dict, advice: dict, study_plan: list, mode: str = "individual") -> str:
    ...
    mode="individual" 个人模式：无分数卡片、板块表格为常考知识点/难点、无答题明细；
    mode="batch" 批量模式：保持原有得分体系不变。"""
    school = meta.get("school", "")
    exam = (meta.get("exam_name") or "").strip()
    personal = mode == "individual"
    if personal:
        title = "个人试卷分析报告"
    else:
    ...
    # 顶部卡片：仅批量模式（总得分 + 选择题得分率 + 非选择题得分率 + 试卷整体难度）
    cards = ""
    if not personal:
        cards = f"""
    <div class="cards">
    ...
    if personal:
        # 个人模式：板块表格 = 高考占比 / 重要性 / 常考知识点 / 难点（配合试卷情况匹配）
        sec_rows = ""
        for s in analysis["sections"]:
            kps = list(dict.fromkeys(q.get("knowledge_point") or "" for q in s["questions"]))
            kps = [k for k in kps if k]
            hot = section_hot_kps(s["key"], kps)
            freq_html = "、".join(hot["frequent"]) or '<span style="color:#99a5b8">—</span>'
            hard_html = "、".join(hot["hard"]) or '<span style="color:#99a5b8">—</span>'
            sec_rows += (f"<tr><td><span class='section-head'>{s['name']}</span></td>"
                         f"<td>{s['gaokao_weight']:.0%}</td><td>{s['importance']}</td>"
                         f"<td style='text-align:left'>{freq_html}</td>"
                         f"<td style='text-align:left'>{hard_html}</td></tr>")
        part1 = f"""
    <h2>一、试卷整体分析</h2>
    <div class="note">{advice.get('paper_comment', '')}</div>
    <table>
      <tr><th>板块</th><th>高考占比</th><th>重要性</th><th>常考知识点</th><th>难点</th></tr>
      {sec_rows}
    </table>
    ...
    # 二、待巩固题目：每个错题一行（题目序号、板块、考察知识点+说明）
    wrong_rows = ""
    ...
    <h2>二、待巩固题目</h2>
    <table>
      <tr><th>题目序号</th><th>板块</th><th>考察知识点</th></tr>
      {wrong_rows or '<tr><td colspan="3">本次考试全部题目均已完成得很好，继续保持！</td></tr>'}
    </table>"""
    else:
        part2 = f"""
    <h2>二、待巩固题目</h2>
