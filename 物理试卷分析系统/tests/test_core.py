...
class TestReportStructure(unittest.TestCase):
    ...
    def test_build_html(self):
        """个人模式报告：无分数卡片、板块表为常考知识点/难点、无答题明细"""
        from backend.report_builder import build_html
        qs = [
            Question(qid="1", section_key="lixue", qtype="single", full_score=4,
                     got_score=0, knowledge_point="牛顿运动定律", wrong=True),
            Question(qid="2", section_key="dianci", qtype="multi", full_score=5,
        ...
        for marker in ["一、试卷整体分析", "二、待巩固题目", "三、强化学习",
                       "四、总结", "常考知识点", "难点"]:
            self.assertIn(marker, html)
        # 个人模式：无分数卡片、无答题明细
        for marker in ["总得分", "选择题得分率", "非选择题得分率", "试卷整体难度",
                       "附：答题明细", "待提升分", "学生答案"]:
    ...
    def test_build_html_batch_score_cards(self):
        """批量模式报告：保留得分卡片与答题明细"""
        from backend.report_builder import build_html
        qs = [
            Question(qid="1", section_key="lixue", qtype="single", full_score=4,
        ...
        advice = {"paper_comment": "评析", "section_importance": "占比",
                  "study_advice": "建议", "encouragement": "加油"}
        html = build_html(analysis, {"teacher": "张老师", "student": "欧阳娜娜"}, advice, [], mode="batch")
        for marker in ["总得分", "选择题得分率", "非选择题得分率", "试卷整体难度",
                       "附：答题明细", "待提升分", "得分率"]:
            self.assertIn(marker, html)
        self.assertNotIn("常考知识点", html)
    ...
    def test_report_exact_scores(self):
        ...
        analysis = analyze(qs)
        advice = {"paper_comment": "", "section_importance": "", "study_advice": "", "encouragement": ""}
        html = build_html(analysis, {"teacher": "张老师", "student": "张三"}, advice, [], mode="batch")
    ...
    def test_personal_wrong_marks(self):
        """个人模式：wrong 标记驱动分析（对=1 题，错=0 题）"""
        from backend.analyzer import analyze
        qs = [
            Question(qid="1", section_key="lixue", qtype="single", full_score=1.0,
                     got_score=1.0, knowledge_point="匀变速直线运动"),
            Question(qid="2", section_key="lixue", qtype="single", full_score=1.0,
                     got_score=0.0, knowledge_point="牛顿运动定律", wrong=True),
            Question(qid="3", section_key="dianci", qtype="multi", full_score=1.0,
                     got_score=1.0, knowledge_point="电磁感应"),
            Question(qid="4", section_key="dianci", qtype="calculation", full_score=1.0,
                     got_score=0.0, knowledge_point="带电粒子在磁场中运动", wrong=True),
        ]
        a = analyze(qs)
        self.assertEqual(a["total_full"], 4)      # 共 4 题
        self.assertEqual(a["total_got"], 2)       # 对 2 题
        self.assertEqual(a["total_lost"], 2)      # 错 2 题
        self.assertEqual(a["overall_rate"], 0.5)
        # 板块：力学对 1 错 1，电磁对 1 错 1
        for s in a["sections"]:
            self.assertEqual(s["full_score"], 2)
            self.assertEqual(s["got_score"], 1)
            self.assertEqual(s["lost_score"], 1)
        # 知识点丢分聚合只含错题知识点
        kps = [k["knowledge_point"] for k in a["knowledge_loss"]]
        self.assertIn("牛顿运动定律", kps)
        self.assertIn("带电粒子在磁场中运动", kps)
        self.assertNotIn("匀变速直线运动", kps)

    def test_personal_report_hot_kps(self):
        """个人报告：板块表格为常考知识点/难点，且配合试卷情况匹配"""
        from backend.report_builder import build_html
        from backend.knowledge import section_hot_kps
        # 匹配：牛顿第二定律→常考+难点；电磁感应→常考+难点
        hot = section_hot_kps("lixue", ["牛顿第二定律", "功与功率"])
        self.assertIn("牛顿第二定律", hot["frequent"])
        self.assertIn("牛顿第二定律", hot["hard"])
        self.assertNotIn("功与功率", hot["frequent"])  # 不在清单内不硬凑
        qs = [
            Question(qid="1", section_key="lixue", qtype="single", full_score=1.0,
                     got_score=0.0, knowledge_point="牛顿第二定律", wrong=True),
            Question(qid="2", section_key="dianci", qtype="multi", full_score=1.0,
                     got_score=1.0, knowledge_point="电磁感应"),
        ]
        analysis = analyze(qs)
        advice = {"paper_comment": "", "section_importance": "", "study_advice": "", "encouragement": ""}
        html = build_html(analysis, {"teacher": "张老师", "student": "张三"}, advice, [])
        self.assertIn("常考知识点", html)
        self.assertIn("牛顿第二定律", html)
        self.assertIn("电磁感应", html)
        self.assertNotIn("功与功率", html)

    def test_personal_report_video_numbered_title(self):
        """个人报告强化学习：视频直接显示带数字编号的完整标题"""
        from backend.report_builder import build_html
        qs = [
            Question(qid="1", section_key="lixue", qtype="single", full_score=1.0,
                     got_score=0.0, knowledge_point="匀变速直线运动", wrong=True),
        ]
        analysis = analyze(qs)
        advice = {"paper_comment": "", "section_importance": "", "study_advice": "", "encouragement": ""}
        plan = [{
            ...
            "questions": [{"qid": "1"}],
            "videos": [{"title": "1.1.1.2初速度为零的匀加速直线运动", "url": ""}],
        }]
        html = build_html(analysis, {"teacher": "张老师", "student": "张三"}, advice, plan)
        self.assertIn("1.1.1.2初速度为零的匀加速直线运动", html)

    def test_template_advice_personal(self):
        """个人模式模板文案：按题数/正确率，不提分数"""
        from backend.llm import template_advice
        qs = [
            Question(qid="1", section_key="lixue", qtype="single", full_score=1.0,
                     got_score=1.0, knowledge_point="匀变速直线运动"),
            Question(qid="2", section_key="dianci", qtype="single", full_score=1.0,
                     got_score=0.0, knowledge_point="电磁感应", wrong=True),
        ]
        advice = template_advice(analyze(qs), student="李明", personal=True)
        self.assertIn("共2道题", advice["paper_comment"])
        self.assertIn("正确率", advice["paper_comment"])
        self.assertNotIn("分，得分", advice["paper_comment"])
        self.assertNotIn("满分", advice["paper_comment"])
        self.assertIn("李明同学", advice["encouragement"])
        # 非个人模式不受影响
        advice2 = template_advice(analyze(qs), student="李明")
        self.assertIn("得分率", advice2["paper_comment"])
        self.assertIn("满分", advice2["paper_comment"])
